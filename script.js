...
function updateLoyaltyPointsDisplay() {
    const pointsSpan = document.getElementById('loyaltyPoints');
...
function bindActivityListeners() { const events = ['click', 'scroll', 'keydown', 'touchstart']; for (let i = 0; i < events.length; i++) { document.addEventListener(events[i], updateActivity); } }
...
function showPage(pageName) {
    ...
    for (let i = 0; i < pages.length; i++) { const el = document.getElementById(pages[i]); if (el) { el.style.display = 'none'; el.classList.add('hidden-page'); } }
...
function closeSidebar() { const s = document.getElementById('sidebar'); if (s) s.classList.remove('open'); const o = document.getElementById('overlay'); if (o) o.classList.remove('active'); }
function openSidebar() { const s = document.getElementById('sidebar'); if (s) s.classList.add('open'); const o = document.getElementById('overlay'); if (o) o.classList.add('active'); }
...
function renderEventCard(event) {
    const hasRated = ratings.some(function(r) { return r.eventId === event.id && r.userWallet === (currentUser.wallet || currentUser.name); });
    const userRating = ratings.find(function(r) { return r.eventId === event.id && r.userWallet === (currentUser.wallet || currentUser.name); });
    let avgRating = 0;
    const eventRatings = ratings.filter(function(r) { return r.eventId === event.id; });
    if (eventRatings.length > 0) { avgRating = eventRatings.reduce(function(a, r) { return a + r.rating; }, 0) / eventRatings.length; }
    const hasTicket = tickets.some(function(t) { return t.eventId === event.id && t.buyerWallet === (currentUser.wallet || currentUser.name); });
    
    let galleryHtml = '';
    if (event.images && event.images.length > 0) {
        galleryHtml = '<div class="event-gallery">';
        for (let i = 0; i < Math.min(event.images.length, 4); i++) {
    ...
    const dateFormatted = dateEvent.toLocaleDateString('fr-FR');
    const timeFormatted = dateEvent.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
    
    let ratingStars = '';
    const fullStars = Math.floor(avgRating);
    for (let i = 0; i < fullStars; i++) ratingStars += '★';
    for (let i = fullStars; i < 5; i++) ratingStars += '☆';
    
    const ratingHtml = avgRating > 0 ? '<span class="event-rating">' + ratingStars + ' ' + avgRating.toFixed(1) + ' (' + eventRatings.length + ')</span>' : '<span class="event-rating">Nouveau</span>';
    
    const descriptionShort = event.description ? event.description.substring(0, 100) + (event.description.length > 100 ? '...' : '') : '';
    
    let ratingButtonHtml = '';
    if (!hasRated && hasTicket) {
        ratingButtonHtml = '<button class="rating-btn" onclick="openRatingModal(\'' + event.id + '\', \'' + escapeHtml(event.title) + '\')">Noter cet evenement</button>';
    } else if (hasRated) {
        ratingButtonHtml = '<div class="rated-badge">Vous avez note ' + (userRating ? userRating.rating : '') + '/5</div>';
    }
...
function renderEventsByCategory() {
    ...
    if (!container) return;
    let filtered = events.filter(function(e) { return (currentFilter === 'Tous' || e.category === currentFilter) && (e.title.toLowerCase().includes(searchQuery) || (e.location && e.location.toLowerCase().includes(searchQuery))); });
    ...
    for (let i = 0; i < cats.length; i++) {
        const cat = cats[i];
        const catEvents = filtered.filter(function(e) { return e.category === cat; });
...
function initFilters() {
    ...
    for (let i = 0; i < chips.length; i++) { chips[i].addEventListener('click', function() { currentFilter = this.dataset.category; initFilters(); renderEventsByCategory(); }); }
}
        ...
        const payment = await Pi.createPayment({
            ...
            onReadyForServerCompletion: function(paymentId, txid) {
                fetch(BACKEND_URL + '/api/pi/complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ paymentId: paymentId, txid: txid, amount: event.price, metadata: { eventId: event.id } }) }).then(function() {
                    const ticketExiste = tickets.some(function(t) { return t.transactionId === txid; });
                    if (!ticketExiste) {
                        event.seatsLeft--;
                        saveEvents();
                        tickets.push({
...
function renderTickets() {
    ...
    if (!container) return;
    let active = tickets.filter(function(t) { return new Date(t.eventDate) > new Date(); });
...
function renderHistory() {
    ...
    if (!container) return;
    let old = tickets.filter(function(t) { return new Date(t.eventDate) <= new Date(); });
...
function createEvent(e) {
    ...
    const imageInputs = document.querySelectorAll('.eventImageUrl');
    let images = [];
    for (let i = 0; i < imageInputs.length; i++) { const url = imageInputs[i].value.trim(); if (url) images.push(url); }
...
function openRatingModal(eventId, eventTitle) {
    ...
    for (let i = 0; i < stars.length; i++) {
        ...
        stars[i].onclick = function() { selectedRating = parseInt(this.dataset.rating); for (let j = 0; j < stars.length; j++) { if (parseInt(stars[j].dataset.rating) <= selectedRating) stars[j].classList.add('active'); else stars[j].classList.remove('active'); } };
    }
...
function renderMyRatings() {
    ...
    if (!container) return;
    const myRatings = ratings.filter(function(r) { return r.userWallet === (currentUser.wallet || currentUser.name); });
    if (!myRatings.length) { container.innerHTML = '<p style="text-align:center;padding:2rem;">Aucune evaluation</p>'; return; }
    container.innerHTML = myRatings.map(function(r) { let stars = ''; for (let i = 0; i < r.rating; i++) stars += '★'; for (let i = r.rating; i < 5; i++) stars += '☆'; return '<div class="ticket-card"><h3>' + escapeHtml(r.eventTitle) + '</h3><div>Note: ' + r.rating + '/5 ' + stars + '</div>' + (r.comment ? '<p>"' + escapeHtml(r.comment) + '"</p>' : '') + '<small>' + new Date(r.date).toLocaleDateString() + '</small></div>'; }).join('');
}
...
function initAdmin() {
    ...
    let clicks = 0;
    if (logo) logo.addEventListener('click', function() { clicks++; if (clicks === 5) { const pwd = prompt('Code admin:'); if (pwd === adminCode) { localStorage.setItem('betix_admin_password', pwd); adminItem.style.display = 'block'; alert('Admin active'); } clicks = 0; } setTimeout(function() { clicks = 0; }, 2000); });
...
function loadAdminPage() {
    ...
    let usersHtml = '<table border="1" cellpadding="5"><tr><th>Utilisateur</th><th>Wallet</th><th>Tickets</th></tr>';
    ...
    for (let i = 0; i < connectedUsers.length; i++) { const u = connectedUsers[i]; usersHtml += '<tr><td>' + escapeHtml(u.name) + 'NonNullable?' + (u.wallet || 'Non connecte') + 'NonNullable?' + (u.ticketCount || 0) + 'NonNullable?' + '</tr>'; }
    usersHtml += '</table';
...
function initChat() {
    ...
    if (!widget) return;
    function load() { if (!msgs) return; msgs.innerHTML = ''; if (!chatMessages.length) add({ text: "Bonjour ! Comment pouvons-nous vous aider ?", sender: 'Support', isUser: false, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }); else for (let i = 0; i < chatMessages.length; i++) add(chatMessages[i]); }
...
function trackUserConnection() {
    if (currentUser.wallet) {
        let existing = null;
        for (let i = 0; i < connectedUsers.length; i++) { if (connectedUsers[i].wallet === currentUser.wallet) { existing = connectedUsers[i]; break; } }
...
function googleTranslateElementInit() {
    ...
    setTimeout(function() {
        ...
        for (let i = 0; i < iframes.length; i++) { const iframe = iframes[i]; if (iframe.className === 'goog-te-banner-frame' || iframe.id === 'google_translate_frame') { iframe.style.display = 'none'; iframe.style.visibility = 'hidden'; iframe.style.height = '0px'; iframe.style.width = '0px'; } }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const menuBtn = document.getElementById('menuBtn'); const closeSidebarBtn = document.getElementById('closeSidebarBtn'); const overlay = document.getElementById('overlay');
    const sidebarWalletBtn = document.getElementById('sidebarWalletBtn'); const profileConnectBtn = document.getElementById('profileConnectBtn');
    const eventForm = document.getElementById('eventForm'); const searchInput = document.getElementById('searchInput'); const clearDataBtn = document.getElementById('clearDataBtn');
    ...
    if (addImageBtn) { addImageBtn.addEventListener('click', function() { const container = document.getElementById('imageUrlsContainer'); const newInput = document.createElement('input'); newInput.type = 'url'; newInput.className = 'eventImageUrl'; newInput.placeholder = 'URL de l\'image'; container.appendChild(newInput); }); }
    const sidebarItems = document.querySelectorAll('.sidebar-item');
    for (let i = 0; i < sidebarItems.length; i++) { sidebarItems[i].addEventListener('click', function() { showPage(this.dataset.page); closeSidebar(); }); }
